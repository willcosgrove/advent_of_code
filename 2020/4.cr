...
PASSPORT_REGEX = /([a-z]{3}):(\S+)/

REQUIRED_FIELDS = %w(byr iyr eyr hgt hcl ecl pid)

passports = STDIN.gets_to_end.split("\n\n").map { |passport_data|
  passport_data.scan(PASSPORT_REGEX).each_with_object({} of String => String) do |match_data, hash|
    hash[match_data[1]] = match_data[2]
  end
}

valid_passport_count = passports.count do |passport|
  REQUIRED_FIELDS.all? { |field| passport.has_key?(field) }
end

puts "Part 1: #{valid_passport_count}"
...
VALIDATIONS = {
  "byr" => ->(str : String) {
    str.matches?(/^\d{4}$/) && (year = str.to_i) && year >= 1920 && year <= 2002
  },
  "iyr" => ->(str : String) {
    str.matches?(/^\d{4}$/) && (year = str.to_i) && year >= 2010 && year <= 2020
  },
  "eyr" => ->(str : String) {
    str.matches?(/^\d{4}$/) && (year = str.to_i) && year >= 2020 && year <= 2030
  },
  "hgt" => ->(str : String) {
    (str.match(/^(\d+)cm$/i) && (height = $~[1].to_i) && height >= 150 && height <= 193) ||
    (str.match(/^(\d+)in$/i) && (height = $~[1].to_i) && height >= 59 && height <= 76)
  },
  "hcl" => ->(str : String) { str.matches?(/^#[0-9a-f]{6}$/i) },
  "ecl" => ->(str : String) { str.in? %w(amb blu brn gry grn hzl oth) },
  "pid" => ->(str : String) { str.matches?(/^\d{9}$/) },
}
...
newly_valid_passport_count = passports.count do |passport|
  VALIDATIONS.all? do |key, validation|
    passport.has_key?(key) && validation.call(passport[key])
  end
end

puts "Part 2: #{newly_valid_passport_count}"
