...
group_answer_sets = groups.map { |group|
  answer_set = Set(Char).new
  group.gsub(/\s/, "").each_char do |answer|
    answer_set << answer
  end
  answer_set
}

puts "Part 1: #{group_answer_sets.sum(&.size)}"
...
intersecting_group_answer_sets = groups.map { |group|
  group.split("\n").map { |seat|
    answer_set = Set(Char).new
    seat.each_char do |answer|
      answer_set << answer
    end
    answer_set
...
puts "Part 2: #{intersecting_group_answer_sets.sum(&.size)}"
