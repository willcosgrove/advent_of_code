...
class XmasVerificationFailed < Exception
  getter failing_element : Int64

  def initialize(@failing_element)
  end
end

def xmas_verify(array : Array(Int64), preamble = 25)
  array.each_with_index do |el, i|
    next if i < preamble

    next if array[(i - preamble)...i].each_combination(2).any? do |(one, two)|
              one + two == el
            end

    raise XmasVerificationFailed.new(el)
  end
end

input = STDIN.each_line.to_a.map(&.to_i64)
failing_element : Int64?

begin
  xmas_verify(input)
rescue e : XmasVerificationFailed
  puts "Part 1: #{e.failing_element}"
  failing_element = e.failing_element
end

exit(1) unless failing_element
...
weakness_set : Array(Int64)? = nil

(2..input.size).each do |cons_size|
  weakness_set = input.each.cons(cons_size).find { |elements|
    elements.sum == failing_element
  }
  break if weakness_set
end

if weakness_set
  puts "Part 2: #{weakness_set.min + weakness_set.max}"
end
