...
input = STDIN.each_line.map(&.to_i).to_a

pair = input.each_combination(2).find do |combo|
  combo.sum == 2020
end

raise "No pair found" unless pair

puts "#{pair.join(" + ")} = 2020"
puts "#{pair.join(" * ")} = #{pair.product}"
...
triad = input.each_combination(3).find do |triple|
  triple.sum == 2020
end

raise "No triad found" unless triad

puts "#{triad.join(" + ")} = 2020"
puts "#{triad.join(" * ")} = #{triad.product}"
